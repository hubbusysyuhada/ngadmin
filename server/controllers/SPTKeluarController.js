...
class SPTKeluarController {
    ...
    static async newSPT (req, res, next) {
        try {
            ...
            const formattedDate = new Date(`${answer.TanggalSurat} 12:00:00`)
            const year = formattedDate.getFullYear()
            let month = formattedDate.getMonth()  + 1
            const months = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember']
            answer.TanggalSurat = `${formattedDate.getDate()} ${months[month - 1]} ${year}`
            if (month < 10) month = '0' + month
            answer.NomorSurat = `ST.${newNumber}/REN/SUBDIT-PWAP/${month}/${year}`
            if (answer.Ditujukan) {
                answer.Ditujukan = answer.Ditujukan.split(',')
                answer.Ditujukan = answer.Ditujukan.map(name => name.trim())
                answer.Ditujukan = JSON.stringify(answer.Ditujukan)
